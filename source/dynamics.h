/* Here we have the expressions of gravitational potentials, accelerations, forces and torques for a couple of dynamical assumptions.
   I know that they are not in the most optimized form as there are repentances, but for know I account for correctness and clarity. :) */

#ifndef DYNAMICS_H
#define DYNAMICS_H

#include<cmath>

#ifdef USE_OPENMP
    #include<omp.h>
#endif

#include"constants.h"
#include"typedef.h"
#include"linalg.h"

/* Gravity potential expressions. */

//Mutual potential of 2 rigid bodies, assuming inertial integral expansion of order 2 approximation.
double mut_pot_integrals_ord2(const dvec3 &r, const double M1, const dtens &J1, const dmat3 &A1,
                                              const double M2, const dtens &J2, const dmat3 &A2)
{
    const double I1x = J1[0][2][0] + J1[0][0][2];
    const double I1y = J1[2][0][0] + J1[0][0][2];
    const double I1z = J1[2][0][0] + J1[0][2][0];

    const double I2x = J2[0][2][0] + J2[0][0][2];
    const double I2y = J2[2][0][0] + J2[0][0][2];
    const double I2z = J2[2][0][0] + J2[0][2][0];

    const dvec3 a1 = {A1[0][0], A1[1][0], A1[2][0]};
    const dvec3 a2 = {A1[0][1], A1[1][1], A1[2][1]};
    const dvec3 a3 = {A1[0][2], A1[1][2], A1[2][2]};

    const dvec3 b1 = {A2[0][0], A2[1][0], A2[2][0]};
    const dvec3 b2 = {A2[0][1], A2[1][1], A2[2][1]};
    const dvec3 b3 = {A2[0][2], A2[1][2], A2[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l1 = dot(ru,a1);
    const double m1 = dot(ru,a2);
    const double n1 = dot(ru,a3);

    const double l2 = dot(ru,b1);
    const double m2 = dot(ru,b2);
    const double n2 = dot(ru,b3);

    //Order 0 (Keplerian).
    const double V0 = -G*M1*M2/d;

    //Order 2.
    const double V2 = -(G*M2/(2*d*d*d))*( (1 - 3*l1*l1)*I1x + (1 - 3*m1*m1)*I1y + (1 - 3*n1*n1)*I1z ) +
                      -(G*M1/(2*d*d*d))*( (1 - 3*l2*l2)*I2x + (1 - 3*m2*m2)*I2y + (1 - 3*n2*n2)*I2z );

    return V0 + V2;
}

//Mutual potential of 2 rigid bodies, assuming inertial integral expansion of order 3 approximation.
double mut_pot_integrals_ord3(const dvec3 &r, const double M1, const dtens &J1, const dmat3 &A1,
                                              const double M2, const dtens &J2, const dmat3 &A2)
{
    const double J1xxx = J1[3][0][0];
    const double J1yyy = J1[0][3][0];
    const double J1zzz = J1[0][0][3];
    const double J1xxy = J1[2][1][0];
    const double J1xyy = J1[1][2][0];
    const double J1xxz = J1[2][0][1];
    const double J1xzz = J1[1][0][2];
    const double J1yyz = J1[0][2][1];
    const double J1yzz = J1[0][1][2];
    const double J1xyz = J1[1][1][1];
    
    const double I1x = J1[0][2][0] + J1[0][0][2];
    const double I1y = J1[2][0][0] + J1[0][0][2];
    const double I1z = J1[2][0][0] + J1[0][2][0];

    const double J2xxx = J2[3][0][0];
    const double J2yyy = J2[0][3][0];
    const double J2zzz = J2[0][0][3];
    const double J2xxy = J2[2][1][0];
    const double J2xyy = J2[1][2][0];
    const double J2xxz = J2[2][0][1];
    const double J2xzz = J2[1][0][2];
    const double J2yyz = J2[0][2][1];
    const double J2yzz = J2[0][1][2];
    const double J2xyz = J2[1][1][1];

    const double I2x = J2[0][2][0] + J2[0][0][2];
    const double I2y = J2[2][0][0] + J2[0][0][2];
    const double I2z = J2[2][0][0] + J2[0][2][0];

    const dvec3 a1 = {A1[0][0], A1[1][0], A1[2][0]};
    const dvec3 a2 = {A1[0][1], A1[1][1], A1[2][1]};
    const dvec3 a3 = {A1[0][2], A1[1][2], A1[2][2]};

    const dvec3 b1 = {A2[0][0], A2[1][0], A2[2][0]};
    const dvec3 b2 = {A2[0][1], A2[1][1], A2[2][1]};
    const dvec3 b3 = {A2[0][2], A2[1][2], A2[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l1 = dot(ru,a1);
    const double m1 = dot(ru,a2);
    const double n1 = dot(ru,a3);

    const double l2 = dot(ru,b1);
    const double m2 = dot(ru,b2);
    const double n2 = dot(ru,b3);

    //Order 0 (Keplerian).
    const double V0 = -G*M1*M2/d;

    //Order 2.
    const double V2 = -(G*M2/(2*d*d*d))*( (1 - 3*l1*l1)*I1x + (1 - 3*m1*m1)*I1y + (1 - 3*n1*n1)*I1z ) +
                      -(G*M1/(2*d*d*d))*( (1 - 3*l2*l2)*I2x + (1 - 3*m2*m2)*I2y + (1 - 3*n2*n2)*I2z );

    //Order 3.
    const double V3 = -(G*M2/(2*d*d*d*d))*(   l1*(5*l1*l1 - 3)*J1xxx +   m1*(5*m1*m1 - 3)*J1yyy +   n1*(5*n1*n1 - 3)*J1zzz +
                                            3*m1*(5*l1*l1 - 1)*J1xxy + 3*l1*(5*m1*m1 - 1)*J1xyy + 3*n1*(5*l1*l1 - 1)*J1xxz +
                                            3*l1*(5*n1*n1 - 1)*J1xzz + 3*n1*(5*m1*m1 - 1)*J1yyz + 3*m1*(5*n1*n1 - 1)*J1yzz +
                                           30*l1*m1*n1*J1xyz ) +
                       (G*M1/(2*d*d*d*d))*(   l2*(5*l2*l2 - 3)*J2xxx +   m2*(5*m2*m2 - 3)*J2yyy +   n2*(5*n2*n2 - 3)*J2zzz +
                                            3*m2*(5*l2*l2 - 1)*J2xxy + 3*l2*(5*m2*m2 - 1)*J2xyy + 3*n2*(5*l2*l2 - 1)*J2xxz +
                                            3*l2*(5*n2*n2 - 1)*J2xzz + 3*n2*(5*m2*m2 - 1)*J2yyz + 3*m2*(5*n2*n2 - 1)*J2yzz +
                                           30*l2*m2*n2*J2xyz );

    return V0 + V2 + V3;
}

//Mutual potential of 2 rigid bodies, assuming inertial integral expansion of order 4 approximation.
double mut_pot_integrals_ord4(const dvec3 &r, const double M1, const dtens &J1, const dmat3 &A1,
                                              const double M2, const dtens &J2, const dmat3 &A2)
{
    const double J1xx = J1[2][0][0];
    const double J1yy = J1[0][2][0];
    const double J1zz = J1[0][0][2];

    const double J1xxx = J1[3][0][0];
    const double J1yyy = J1[0][3][0];
    const double J1zzz = J1[0][0][3];
    const double J1xxy = J1[2][1][0];
    const double J1xyy = J1[1][2][0];
    const double J1xxz = J1[2][0][1];
    const double J1xzz = J1[1][0][2];
    const double J1yyz = J1[0][2][1];
    const double J1yzz = J1[0][1][2];
    const double J1xyz = J1[1][1][1];

    const double J1xxxx = J1[4][0][0];
    const double J1yyyy = J1[0][4][0];
    const double J1zzzz = J1[0][0][4];
    const double J1xxxy = J1[3][1][0];
    const double J1xyyy = J1[1][3][0];
    const double J1xxxz = J1[3][0][1];
    const double J1xzzz = J1[1][0][3];
    const double J1yyyz = J1[0][3][1];
    const double J1yzzz = J1[0][1][3];
    const double J1xxyy = J1[2][2][0];
    const double J1xxzz = J1[2][0][2];
    const double J1yyzz = J1[0][2][2];
    const double J1xxyz = J1[2][1][1];
    const double J1xyyz = J1[1][2][1];
    const double J1xyzz = J1[1][1][2];
    
    const double I1x = J1[0][2][0] + J1[0][0][2];
    const double I1y = J1[2][0][0] + J1[0][0][2];
    const double I1z = J1[2][0][0] + J1[0][2][0];

    /////////////////////////////////////////////////////////////////////////////////

    const double J2xx = J2[2][0][0];
    const double J2yy = J2[0][2][0];
    const double J2zz = J2[0][0][2];

    const double J2xxx = J2[3][0][0];
    const double J2yyy = J2[0][3][0];
    const double J2zzz = J2[0][0][3];
    const double J2xxy = J2[2][1][0];
    const double J2xyy = J2[1][2][0];
    const double J2xxz = J2[2][0][1];
    const double J2xzz = J2[1][0][2];
    const double J2yyz = J2[0][2][1];
    const double J2yzz = J2[0][1][2];
    const double J2xyz = J2[1][1][1];

    const double J2xxxx = J2[4][0][0];
    const double J2yyyy = J2[0][4][0];
    const double J2zzzz = J2[0][0][4];
    const double J2xxxy = J2[3][1][0];
    const double J2xyyy = J2[1][3][0];
    const double J2xxxz = J2[3][0][1];
    const double J2xzzz = J2[1][0][3];
    const double J2yyyz = J2[0][3][1];
    const double J2yzzz = J2[0][1][3];
    const double J2xxyy = J2[2][2][0];
    const double J2xxzz = J2[2][0][2];
    const double J2yyzz = J2[0][2][2];
    const double J2xxyz = J2[2][1][1];
    const double J2xyyz = J2[1][2][1];
    const double J2xyzz = J2[1][1][2];
    
    const double I2x = J2[0][2][0] + J2[0][0][2];
    const double I2y = J2[2][0][0] + J2[0][0][2];
    const double I2z = J2[2][0][0] + J2[0][2][0];
    
    const dvec3 a1 = {A1[0][0], A1[1][0], A1[2][0]};
    const dvec3 a2 = {A1[0][1], A1[1][1], A1[2][1]};
    const dvec3 a3 = {A1[0][2], A1[1][2], A1[2][2]};

    const dvec3 b1 = {A2[0][0], A2[1][0], A2[2][0]};
    const dvec3 b2 = {A2[0][1], A2[1][1], A2[2][1]};
    const dvec3 b3 = {A2[0][2], A2[1][2], A2[2][2]};

    const dmat3 C = dot(transpose(A1), A2);
    const double c11 = C[0][0], c12 = C[0][1], c13 = C[0][2];
    const double c21 = C[1][0], c22 = C[1][1], c23 = C[1][2];
    const double c31 = C[2][0], c32 = C[2][1], c33 = C[2][2];

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l1 = dot(ru,a1);
    const double m1 = dot(ru,a2);
    const double n1 = dot(ru,a3);

    const double l2 = dot(ru,b1);
    const double m2 = dot(ru,b2);
    const double n2 = dot(ru,b3);

    //Order 0 (Keplerian).
    const double V0 = -G*M1*M2/d;

    //Order 2.
    const double V2 = -(G*M2/(2*d*d*d))*( (1 - 3*l1*l1)*I1x + (1 - 3*m1*m1)*I1y + (1 - 3*n1*n1)*I1z ) +
                      -(G*M1/(2*d*d*d))*( (1 - 3*l2*l2)*I2x + (1 - 3*m2*m2)*I2y + (1 - 3*n2*n2)*I2z );

    //Order 3.
    const double V3 = -(G*M2/(2*d*d*d*d))*(   l1*(5*l1*l1 - 3)*J1xxx +   m1*(5*m1*m1 - 3)*J1yyy +   n1*(5*n1*n1 - 3)*J1zzz +
                                            3*m1*(5*l1*l1 - 1)*J1xxy + 3*l1*(5*m1*m1 - 1)*J1xyy + 3*n1*(5*l1*l1 - 1)*J1xxz +
                                            3*l1*(5*n1*n1 - 1)*J1xzz + 3*n1*(5*m1*m1 - 1)*J1yyz + 3*m1*(5*n1*n1 - 1)*J1yzz +
                                           30*l1*m1*n1*J1xyz ) +
                       (G*M1/(2*d*d*d*d))*(   l2*(5*l2*l2 - 3)*J2xxx +   m2*(5*m2*m2 - 3)*J2yyy +   n2*(5*n2*n2 - 3)*J2zzz +
                                            3*m2*(5*l2*l2 - 1)*J2xxy + 3*l2*(5*m2*m2 - 1)*J2xyy + 3*n2*(5*l2*l2 - 1)*J2xxz +
                                            3*l2*(5*n2*n2 - 1)*J2xzz + 3*n2*(5*m2*m2 - 1)*J2yyz + 3*m2*(5*n2*n2 - 1)*J2yzz +
                                           30*l2*m2*n2*J2xyz );
    
    //Order 4.
    const double V4 = -(G*M2/(8*d*d*d*d*d))*( (35*pow(l1,4) - 30*l1*l1 + 3)*J1xxxx + (35*pow(m1,4) - 30*m1*m1 + 3)*J1yyyy + (35*pow(n1,4) - 30*n1*n1 + 3)*J1zzzz +
                                               20*l1*m1*(7*l1*l1 - 3)*J1xxxy + 20*l1*m1*(7*m1*m1 - 3)*J1xyyy +
                                               20*l1*n1*(7*l1*l1 - 3)*J1xxxz + 20*l1*n1*(7*n1*n1 - 3)*J1xzzz +
                                               20*m1*n1*(7*m1*m1 - 3)*J1yyyz + 20*m1*n1*(7*n1*n1 - 3)*J1yzzz +
                                               6*(35*l1*l1*m1*m1 - 5*(l1*l1 + m1*m1) + 1)*J1xxyy +
                                               6*(35*l1*l1*n1*n1 - 5*(l1*l1 + n1*n1) + 1)*J1xxzz +
                                               6*(35*m1*m1*n1*n1 - 5*(m1*m1 + n1*n1) + 1)*J1yyzz +
                                               60*m1*n1*(7*l1*l1 - 1)*J1xxyz + 60*l1*n1*(7*m1*m1 - 1)*J1xyyz + 60*l1*m1*(7*n1*n1 - 1)*J1xyzz ) +
                      -(G*M1/(8*d*d*d*d*d))*( (35*pow(l2,4) - 30*l2*l2 + 3)*J2xxxx + (35*pow(m2,4) - 30*m2*m2 + 3)*J2yyyy + (35*pow(n2,4) - 30*n2*n2 + 3)*J2zzzz +
                                               20*l2*m2*(7*l2*l2 - 3)*J2xxxy + 20*l2*m2*(7*m2*m2 - 3)*J2xyyy +
                                               20*l2*n2*(7*l2*l2 - 3)*J2xxxz + 20*l2*n2*(7*n2*n2 - 3)*J2xzzz +
                                               20*m2*n2*(7*m2*m2 - 3)*J2yyyz + 20*m2*n2*(7*n2*n2 - 3)*J2yzzz +
                                               6*(35*l2*l2*m2*m2 - 5*(l2*l2 + m2*m2) + 1)*J2xxyy +
                                               6*(35*l2*l2*n2*n2 - 5*(l2*l2 + n2*n2) + 1)*J2xxzz +
                                               6*(35*m2*m2*n2*n2 - 5*(m2*m2 + n2*n2) + 1)*J2yyzz +
                                               60*m2*n2*(7*l2*l2 - 1)*J2xxyz + 60*l2*n2*(7*m2*m2 - 1)*J2xyyz + 60*l2*m2*(7*n2*n2 - 1)*J2xyzz ) +
                      -(G/(8*d*d*d*d*d))*( (6 + 12*c11*c11 - 30*(l1*l1 + l2*l2) - 120*c11*l1*l2 + 210*l1*l1*l2*l2)*J1xx*J2xx +
                                           (6 + 12*c22*c22 - 30*(m1*m1 + m2*m2) - 120*c22*m1*m2 + 210*m1*m1*m2*m2)*J1yy*J2yy +
                                           (6 + 12*c33*c33 - 30*(n1*n1 + n2*n2) - 120*c33*n1*n2 + 210*n1*n1*n2*n2)*J1zz*J2zz +
                                           (6 + 12*c12*c12 - 30*(l1*l1 + m2*m2) - 120*c12*l1*m2 + 210*l1*l1*m2*m2)*J1xx*J2yy +
                                           (6 + 12*c21*c21 - 30*(l2*l2 + m1*m1) - 120*c21*l2*m1 + 210*l2*l2*m1*m1)*J1yy*J2xx +
                                           (6 + 12*c13*c13 - 30*(l1*l1 + n2*n2) - 120*c13*l1*n2 + 210*l1*l1*n2*n2)*J1xx*J2zz +
                                           (6 + 12*c31*c31 - 30*(l2*l2 + n1*n1) - 120*c31*l2*n1 + 210*l2*l2*n1*n1)*J1zz*J2xx +
                                           (6 + 12*c23*c23 - 30*(m1*m1 + n2*n2) - 120*c23*m1*n2 + 210*m1*m1*n2*n2)*J1yy*J2zz +
                                           (6 + 12*c32*c32 - 30*(m2*m2 + n1*n1) - 120*c32*m2*n1 + 210*m2*m2*n1*n1)*J1zz*J2yy );

    return V0 + V2 + V3 + V4;
}

//Mutual potential of 2 rigid bodies, assuming mascon distributions with constant densities.
double mut_pot_masc(const dvec3 &r, const double M1, const dmatnx3 &masc1, const dmat3 &A1,
                                    const double M2, const dmatnx3 &masc2, const dmat3 &A2)
{
    #ifdef _OPENMP
        const int total_threads = omp_get_max_threads();
        const int used_threads  = (total_threads > 1 ? total_threads/2 : 1);
    #else
        constexpr int used_threads = 1;
    #endif
    (void)used_threads;

    size_t i,j, N1 = masc1.size(), N2 = masc2.size();
    double sum = 0.0;
    #ifdef _OPENMP
        #pragma omp parallel for reduction(+:sum)\
                                 schedule(dynamic)\
                                 num_threads(used_threads)
    #endif
    for (i = 0; i < N1; ++i)
    {
        const dvec3 a1i = dot(A1, masc1[i]);
        for (j = 0; j < N2; ++j)
            sum += 1.0/length(r + dot(A2, masc2[j]) - a1i);
    }
    return -G*M1*M2*sum/double(N1*N2);
}

//Potential of a rigid body upon a test particle at position r, assuming integral expansion of order 2 approximation.
double pot_integrals_ord2(const dvec3 &r, const double M, const dtens &J, const dmat3 &A)
{
    const double Ix = J[0][2][0] + J[0][0][2];
    const double Iy = J[2][0][0] + J[0][0][2];
    const double Iz = J[2][0][0] + J[0][2][0];

    const dvec3 a1 = {A[0][0], A[1][0], A[2][0]};
    const dvec3 a2 = {A[0][1], A[1][1], A[2][1]};
    const dvec3 a3 = {A[0][2], A[1][2], A[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l = dot(ru,a1);
    const double m = dot(ru,a2);
    const double n = dot(ru,a3);

    //Order 0 (Keplerian).
    const double V0 = -G*M/d;

    //Order 2.
    const double V2 = -(G/(2*d*d*d))*( (1 - 3*l*l)*Ix + (1 - 3*m*m)*Iy + (1 - 3*n*n)*Iz );

    return V0 + V2;
}

//Potential of a rigid body upon a test particle at position r, assuming integral expansion of order 3 approximation.
double pot_integrals_ord3(const dvec3 &r, const double M, const dtens &J, const dmat3 &A)
{
    const double Jxxx = J[3][0][0];
    const double Jyyy = J[0][3][0];
    const double Jzzz = J[0][0][3];
    const double Jxxy = J[2][1][0];
    const double Jxyy = J[1][2][0];
    const double Jxxz = J[2][0][1];
    const double Jxzz = J[1][0][2];
    const double Jyyz = J[0][2][1];
    const double Jyzz = J[0][1][2];
    const double Jxyz = J[1][1][1];
    
    const double Ix = J[0][2][0] + J[0][0][2];
    const double Iy = J[2][0][0] + J[0][0][2];
    const double Iz = J[2][0][0] + J[0][2][0];

    const dvec3 a1 = {A[0][0], A[1][0], A[2][0]};
    const dvec3 a2 = {A[0][1], A[1][1], A[2][1]};
    const dvec3 a3 = {A[0][2], A[1][2], A[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l = dot(ru,a1);
    const double m = dot(ru,a2);
    const double n = dot(ru,a3);

    //Order 0 (Keplerian).
    const double V0 = -G*M/d;

    //Order 2.
    const double V2 = -(G/(2*d*d*d))*( (1 - 3*l*l)*Ix + (1 - 3*m*m)*Iy + (1 - 3*n*n)*Iz );

    //Order 3.
    const double V3 = -(G/(2*d*d*d*d))*(    l*(5*l*l - 3)*Jxxx +   m*(5*m*m - 3)*Jyyy +   n*(5*n*n - 3)*Jzzz +
                                          3*m*(5*l*l - 1)*Jxxy + 3*l*(5*m*m - 1)*Jxyy + 3*n*(5*l*l - 1)*Jxxz +
                                          3*l*(5*n*n - 1)*Jxzz + 3*n*(5*m*m - 1)*Jyyz + 3*m*(5*n*n - 1)*Jyzz +
                                         30*l*m*n*Jxyz );

    return V0 + V2 + V3;
}

//Potential of a rigid body upon a test particle at position r, assuming integral expansion of order 4 approximation.
double pot_integrals_ord4(const dvec3 &r, const double M, const dtens &J, const dmat3 &A)
{
    const double Jxxx = J[3][0][0];
    const double Jyyy = J[0][3][0];
    const double Jzzz = J[0][0][3];
    const double Jxxy = J[2][1][0];
    const double Jxyy = J[1][2][0];
    const double Jxxz = J[2][0][1];
    const double Jxzz = J[1][0][2];
    const double Jyyz = J[0][2][1];
    const double Jyzz = J[0][1][2];
    const double Jxyz = J[1][1][1];

    const double Jxxxx = J[4][0][0];
    const double Jyyyy = J[0][4][0];
    const double Jzzzz = J[0][0][4];
    const double Jxxxy = J[3][1][0];
    const double Jxyyy = J[1][3][0];
    const double Jxxxz = J[3][0][1];
    const double Jxzzz = J[1][0][3];
    const double Jyyyz = J[0][3][1];
    const double Jyzzz = J[0][1][3];
    const double Jxxyy = J[2][2][0];
    const double Jxxzz = J[2][0][2];
    const double Jyyzz = J[0][2][2];
    const double Jxxyz = J[2][1][1];
    const double Jxyyz = J[1][2][1];
    const double Jxyzz = J[1][1][2];
    
    const double Ix = J[0][2][0] + J[0][0][2];
    const double Iy = J[2][0][0] + J[0][0][2];
    const double Iz = J[2][0][0] + J[0][2][0];

    const dvec3 a1 = {A[0][0], A[1][0], A[2][0]};
    const dvec3 a2 = {A[0][1], A[1][1], A[2][1]};
    const dvec3 a3 = {A[0][2], A[1][2], A[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l = dot(ru,a1);
    const double m = dot(ru,a2);
    const double n = dot(ru,a3);

    //Order 0 (Keplerian).
    const double V0 = -G*M/d;

    //Order 2.
    const double V2 = -(G/(2*d*d*d))*( (1 - 3*l*l)*Ix + (1 - 3*m*m)*Iy + (1 - 3*n*n)*Iz );

    //Order 3.
    const double V3 = -(G/(2*d*d*d*d))*(    l*(5*l*l - 3)*Jxxx +   m*(5*m*m - 3)*Jyyy +   n*(5*n*n - 3)*Jzzz +
                                          3*m*(5*l*l - 1)*Jxxy + 3*l*(5*m*m - 1)*Jxyy + 3*n*(5*l*l - 1)*Jxxz +
                                          3*l*(5*n*n - 1)*Jxzz + 3*n*(5*m*m - 1)*Jyyz + 3*m*(5*n*n - 1)*Jyzz +
                                         30*l*m*n*Jxyz );

    //Order 4.
    const double V4 = -(G/(8*d*d*d*d*d))*( (35*pow(l,4) - 30*l*l + 3)*Jxxxx + (35*pow(m,4) - 30*m*m + 3)*Jyyyy + (35*pow(n,4) - 30*n*n + 3)*Jzzzz +
                                            20*l*m*(7*l*l - 3)*Jxxxy + 20*l*m*(7*m*m - 3)*Jxyyy +
                                            20*l*n*(7*l*l - 3)*Jxxxz + 20*l*n*(7*n*n - 3)*Jxzzz +
                                            20*m*n*(7*m*m - 3)*Jyyyz + 20*m*n*(7*n*n - 3)*Jyzzz +
                                            6*(35*l*l*m*m - 5*(l*l + m*m) + 1)*Jxxyy +
                                            6*(35*l*l*n*n - 5*(l*l + n*n) + 1)*Jxxzz +
                                            6*(35*m*m*n*n - 5*(m*m + n*n) + 1)*Jyyzz +
                                            60*m*n*(7*l*l - 1)*Jxxyz + 60*l*n*(7*m*m - 1)*Jxyyz + 60*l*m*(7*n*n - 1)*Jxyzz );

    return V0 + V2 + V3 + V4;
}

//Potential of a rigid body upon a test particle at position r, assuming mascons distribution with constant density.
double pot_masc(const dvec3 &r, const double M, const dmatnx3 &masc, const dmat3 &A)
{
    #ifdef _OPENMP
        const int total_threads = omp_get_max_threads();
        const int used_threads  = (total_threads > 1 ? total_threads/2 : 1);
    #else
        constexpr int used_threads = 1;
    #endif
    (void)used_threads;

    size_t i, N = masc.size();
    double sum = 0.0;
    #ifdef _OPENMP
        #pragma omp parallel for reduction(+:sum)\
                                 schedule(dynamic)\
                                 num_threads(used_threads)
    #endif
    for (i = 0; i < N; ++i)
        sum += 1.0/length(r - dot(A, masc[i]));
    return -G*M*sum/double(N);
}

/* End of gravity potential expressions. */

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/* Gravity force and torque expressions. */

//Mutual force of 2 rigid bodies AND the torque felt by the primary, assuming inertial integral expansion of order 2 approximation.
dvec6 mut_force_torque1_integrals_ord2(const dvec3 &r, const double M1, const dtens &J1, const dmat3 &A1,
                                                       const double M2, const dtens &J2, const dmat3 &A2)
{   
    const double I1x = J1[0][2][0] + J1[0][0][2];
    const double I1y = J1[2][0][0] + J1[0][0][2];
    const double I1z = J1[2][0][0] + J1[0][2][0];

    const double I2x = J2[0][2][0] + J2[0][0][2];
    const double I2y = J2[2][0][0] + J2[0][0][2];
    const double I2z = J2[2][0][0] + J2[0][2][0];

    const dvec3 a1 = {A1[0][0], A1[1][0], A1[2][0]};
    const dvec3 a2 = {A1[0][1], A1[1][1], A1[2][1]};
    const dvec3 a3 = {A1[0][2], A1[1][2], A1[2][2]};

    const dvec3 b1 = {A2[0][0], A2[1][0], A2[2][0]};
    const dvec3 b2 = {A2[0][1], A2[1][1], A2[2][1]};
    const dvec3 b3 = {A2[0][2], A2[1][2], A2[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l1 = dot(ru,a1);
    const double m1 = dot(ru,a2);
    const double n1 = dot(ru,a3);

    const double l2 = dot(ru,b1);
    const double m2 = dot(ru,b2);
    const double n2 = dot(ru,b3);

    //order 0 (Keplerian)
    const double V0 = -G*M1*M2/d;

    //order 2
    const double V2 = -(G*M2/(2*d*d*d))*( (1 - 3*l1*l1)*I1x + (1 - 3*m1*m1)*I1y + (1 - 3*n1*n1)*I1z ) +
                      -(G*M1/(2*d*d*d))*( (1 - 3*l2*l2)*I2x + (1 - 3*m2*m2)*I2y + (1 - 3*n2*n2)*I2z );

    const double dV_dd = -V0/d - 3*V2/d;
    const dvec3 dd_dr  = ru;

    const double dV_dl1 = 3*G*I1x*M2*l1/(d*d*d);
    const double dV_dm1 = 3*G*I1y*M2*m1/(d*d*d);
    const double dV_dn1 = 3*G*I1z*M2*n1/(d*d*d);
    const dvec3 dl1_dr  = (a1*d-r*l1)/(d*d);
    const dvec3 dm1_dr  = (a2*d-r*m1)/(d*d);
    const dvec3 dn1_dr  = (a3*d-r*n1)/(d*d);

    const double dV_dl2 = 3*G*I2x*M1*l2/(d*d*d);
    const double dV_dm2 = 3*G*I2y*M1*m2/(d*d*d);
    const double dV_dn2 = 3*G*I2z*M1*n2/(d*d*d);
    const dvec3 dl2_dr  = (b1*d-r*l2)/(d*d);
    const dvec3 dm2_dr  = (b2*d-r*m2)/(d*d);
    const dvec3 dn2_dr  = (b3*d-r*n2)/(d*d);

    //F = -grad(V)
    const dvec3 force = -(dV_dd*dd_dr + dV_dl1*dl1_dr + dV_dm1*dm1_dr + dV_dn1*dn1_dr +
                                        dV_dl2*dl2_dr + dV_dm2*dm2_dr + dV_dn2*dn2_dr);

    const dvec3 dl1_da1 = ru, dm1_da2 = ru, dn1_da3 = ru;
    const dvec3 dV_da1 = dV_dl1*dl1_da1;
    const dvec3 dV_da2 = dV_dm1*dm1_da2;
    const dvec3 dV_da3 = dV_dn1*dn1_da3;

    const dvec3 tau1 = -cross(a1, dV_da1) - cross(a2, dV_da2) - cross(a3, dV_da3);

    return {force[0],force[1],force[2], tau1[0],tau1[1],tau1[2]};
}

//Mutual force of 2 rigid bodies AND the torque felt by the primary, assuming inertial integral expansion of order 3 approximation.
dvec6 mut_force_torque1_integrals_ord3(const dvec3 &r, const double M1, const dtens &J1, const dmat3 &A1,
                                                       const double M2, const dtens &J2, const dmat3 &A2)
{
    const double J1xxx = J1[3][0][0];
    const double J1yyy = J1[0][3][0];
    const double J1zzz = J1[0][0][3];
    const double J1xxy = J1[2][1][0];
    const double J1xyy = J1[1][2][0];
    const double J1xxz = J1[2][0][1];
    const double J1xzz = J1[1][0][2];
    const double J1yyz = J1[0][2][1];
    const double J1yzz = J1[0][1][2];
    const double J1xyz = J1[1][1][1];
    
    const double I1x = J1[0][2][0] + J1[0][0][2];
    const double I1y = J1[2][0][0] + J1[0][0][2];
    const double I1z = J1[2][0][0] + J1[0][2][0];

    const double J2xxx = J2[3][0][0];
    const double J2yyy = J2[0][3][0];
    const double J2zzz = J2[0][0][3];
    const double J2xxy = J2[2][1][0];
    const double J2xyy = J2[1][2][0];
    const double J2xxz = J2[2][0][1];
    const double J2xzz = J2[1][0][2];
    const double J2yyz = J2[0][2][1];
    const double J2yzz = J2[0][1][2];
    const double J2xyz = J2[1][1][1];

    const double I2x = J2[0][2][0] + J2[0][0][2];
    const double I2y = J2[2][0][0] + J2[0][0][2];
    const double I2z = J2[2][0][0] + J2[0][2][0];

    const dvec3 a1 = {A1[0][0], A1[1][0], A1[2][0]};
    const dvec3 a2 = {A1[0][1], A1[1][1], A1[2][1]};
    const dvec3 a3 = {A1[0][2], A1[1][2], A1[2][2]};

    const dvec3 b1 = {A2[0][0], A2[1][0], A2[2][0]};
    const dvec3 b2 = {A2[0][1], A2[1][1], A2[2][1]};
    const dvec3 b3 = {A2[0][2], A2[1][2], A2[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l1 = dot(ru,a1);
    const double m1 = dot(ru,a2);
    const double n1 = dot(ru,a3);

    const double l2 = dot(ru,b1);
    const double m2 = dot(ru,b2);
    const double n2 = dot(ru,b3);

    //Order 0 (Keplerian).
    const double V0 = -G*M1*M2/d;

    //Order 2.
    const double V2 = -(G*M2/(2*d*d*d))*( (1 - 3*l1*l1)*I1x + (1 - 3*m1*m1)*I1y + (1 - 3*n1*n1)*I1z ) +
                      -(G*M1/(2*d*d*d))*( (1 - 3*l2*l2)*I2x + (1 - 3*m2*m2)*I2y + (1 - 3*n2*n2)*I2z );

    //Order 3.
    const double V3 = -(G*M2/(2*d*d*d*d))*(   l1*(5*l1*l1 - 3)*J1xxx +   m1*(5*m1*m1 - 3)*J1yyy +   n1*(5*n1*n1 - 3)*J1zzz +
                                            3*m1*(5*l1*l1 - 1)*J1xxy + 3*l1*(5*m1*m1 - 1)*J1xyy + 3*n1*(5*l1*l1 - 1)*J1xxz +
                                            3*l1*(5*n1*n1 - 1)*J1xzz + 3*n1*(5*m1*m1 - 1)*J1yyz + 3*m1*(5*n1*n1 - 1)*J1yzz +
                                           30*l1*m1*n1*J1xyz ) +
                       (G*M1/(2*d*d*d*d))*(   l2*(5*l2*l2 - 3)*J2xxx +   m2*(5*m2*m2 - 3)*J2yyy +   n2*(5*n2*n2 - 3)*J2zzz +
                                            3*m2*(5*l2*l2 - 1)*J2xxy + 3*l2*(5*m2*m2 - 1)*J2xyy + 3*n2*(5*l2*l2 - 1)*J2xxz +
                                            3*l2*(5*n2*n2 - 1)*J2xzz + 3*n2*(5*m2*m2 - 1)*J2yyz + 3*m2*(5*n2*n2 - 1)*J2yzz +
                                           30*l2*m2*n2*J2xyz );

    const double dV_dd = -V0/d - 3*V2/d - 4*V3/d;
    const dvec3 dd_dr  = r/d;

    const double dV_dl1 = 3*G*I1x*M2*l1/(d*d*d) - G*M2*(10*J1xxx*l1*l1 + J1xxx*(5*l1*l1 - 3) + 30*J1xxy*l1*m1 + 30*J1xxz*l1*n1 + 3*J1xyy*(5*m1*m1 - 1) + 30*J1xyz*m1*n1 + 3*J1xzz*(5*n1*n1 - 1))/(2*d*d*d*d);
    const double dV_dm1 = 3*G*I1y*M2*m1/(d*d*d) - G*M2*(3*J1xxy*(5*l1*l1 - 1) + 30*J1xyy*l1*m1 + 30*J1xyz*l1*n1 + 10*J1yyy*m1*m1 + J1yyy*(5*m1*m1 - 3) + 30*J1yyz*m1*n1 + 3*J1yzz*(5*n1*n1 - 1))/(2*d*d*d*d);
    const double dV_dn1 = 3*G*I1z*M2*n1/(d*d*d) - G*M2*(3*J1xxz*(5*l1*l1 - 1) + 30*J1xyz*l1*m1 + 30*J1xzz*l1*n1 + 3*J1yyz*(5*m1*m1 - 1) + 30*J1yzz*m1*n1 + 10*J1zzz*n1*n1 + J1zzz*(5*n1*n1 - 3))/(2*d*d*d*d);
    const dvec3 dl1_dr  = (a1*d - r*l1)/(d*d);
    const dvec3 dm1_dr  = (a2*d - r*m1)/(d*d);
    const dvec3 dn1_dr  = (a3*d - r*n1)/(d*d);

    const double dV_dl2 = 3*G*I2x*M1*l2/(d*d*d) + G*M1*(10*J2xxx*l2*l2 + J2xxx*(5*l2*l2 - 3) + 30*J2xxy*l2*m2 + 30*J2xxz*l2*n2 + 3*J2xyy*(5*m2*m2 - 1) + 30*J2xyz*m2*n2 + 3*J2xzz*(5*n2*n2 - 1))/(2*d*d*d*d);
    const double dV_dm2 = 3*G*I2y*M1*m2/(d*d*d) + G*M1*(3*J2xxy*(5*l2*l2 - 1) + 30*J2xyy*l2*m2 + 30*J2xyz*l2*n2 + 10*J2yyy*m2*m2 + J2yyy*(5*m2*m2 - 3) + 30*J2yyz*m2*n2 + 3*J2yzz*(5*n2*n2 - 1))/(2*d*d*d*d);
    const double dV_dn2 = 3*G*I2z*M1*n2/(d*d*d) + G*M1*(3*J2xxz*(5*l2*l2 - 1) + 30*J2xyz*l2*m2 + 30*J2xzz*l2*n2 + 3*J2yyz*(5*m2*m2 - 1) + 30*J2yzz*m2*n2 + 10*J2zzz*n2*n2 + J2zzz*(5*n2*n2 - 3))/(2*d*d*d*d);
    const dvec3 dl2_dr  = (b1*d - r*l2)/(d*d);
    const dvec3 dm2_dr  = (b2*d - r*m2)/(d*d);
    const dvec3 dn2_dr  = (b3*d - r*n2)/(d*d);

    //F = -grad(V)
    const dvec3 force = -(dV_dd*dd_dr + dV_dl1*dl1_dr + dV_dm1*dm1_dr + dV_dn1*dn1_dr +
                                        dV_dl2*dl2_dr + dV_dm2*dm2_dr + dV_dn2*dn2_dr);

    const dvec3 dl1_da1 = ru, dm1_da2 = ru, dn1_da3 = ru;
    const dvec3 dV_da1 = dV_dl1*dl1_da1;
    const dvec3 dV_da2 = dV_dm1*dm1_da2;
    const dvec3 dV_da3 = dV_dn1*dn1_da3;

    const dvec3 tau1 = -cross(a1, dV_da1) - cross(a2, dV_da2) - cross(a3, dV_da3);

    return {force[0],force[1],force[2], tau1[0],tau1[1],tau1[2]};
}

//Mutual force of 2 rigid bodies AND the torque felt by the primary, assuming inertial integral expansion of order 4 approximation.
dvec6 mut_force_torque1_integrals_ord4(const dvec3 &r, const double M1, const dtens &J1, const dmat3 &A1,
                                                       const double M2, const dtens &J2, const dmat3 &A2)
{
    const double J1xx = J1[2][0][0];
    const double J1yy = J1[0][2][0];
    const double J1zz = J1[0][0][2];

    const double J1xxx = J1[3][0][0];
    const double J1yyy = J1[0][3][0];
    const double J1zzz = J1[0][0][3];
    const double J1xxy = J1[2][1][0];
    const double J1xyy = J1[1][2][0];
    const double J1xxz = J1[2][0][1];
    const double J1xzz = J1[1][0][2];
    const double J1yyz = J1[0][2][1];
    const double J1yzz = J1[0][1][2];
    const double J1xyz = J1[1][1][1];

    const double J1xxxx = J1[4][0][0];
    const double J1yyyy = J1[0][4][0];
    const double J1zzzz = J1[0][0][4];
    const double J1xxxy = J1[3][1][0];
    const double J1xyyy = J1[1][3][0];
    const double J1xxxz = J1[3][0][1];
    const double J1xzzz = J1[1][0][3];
    const double J1yyyz = J1[0][3][1];
    const double J1yzzz = J1[0][1][3];
    const double J1xxyy = J1[2][2][0];
    const double J1xxzz = J1[2][0][2];
    const double J1yyzz = J1[0][2][2];
    const double J1xxyz = J1[2][1][1];
    const double J1xyyz = J1[1][2][1];
    const double J1xyzz = J1[1][1][2];
    
    const double I1x = J1[0][2][0] + J1[0][0][2];
    const double I1y = J1[2][0][0] + J1[0][0][2];
    const double I1z = J1[2][0][0] + J1[0][2][0];

    /////////////////////////////////////////////////////////////////////////////////

    const double J2xx = J2[2][0][0];
    const double J2yy = J2[0][2][0];
    const double J2zz = J2[0][0][2];

    const double J2xxx = J2[3][0][0];
    const double J2yyy = J2[0][3][0];
    const double J2zzz = J2[0][0][3];
    const double J2xxy = J2[2][1][0];
    const double J2xyy = J2[1][2][0];
    const double J2xxz = J2[2][0][1];
    const double J2xzz = J2[1][0][2];
    const double J2yyz = J2[0][2][1];
    const double J2yzz = J2[0][1][2];
    const double J2xyz = J2[1][1][1];

    const double J2xxxx = J2[4][0][0];
    const double J2yyyy = J2[0][4][0];
    const double J2zzzz = J2[0][0][4];
    const double J2xxxy = J2[3][1][0];
    const double J2xyyy = J2[1][3][0];
    const double J2xxxz = J2[3][0][1];
    const double J2xzzz = J2[1][0][3];
    const double J2yyyz = J2[0][3][1];
    const double J2yzzz = J2[0][1][3];
    const double J2xxyy = J2[2][2][0];
    const double J2xxzz = J2[2][0][2];
    const double J2yyzz = J2[0][2][2];
    const double J2xxyz = J2[2][1][1];
    const double J2xyyz = J2[1][2][1];
    const double J2xyzz = J2[1][1][2];
    
    const double I2x = J2[0][2][0] + J2[0][0][2];
    const double I2y = J2[2][0][0] + J2[0][0][2];
    const double I2z = J2[2][0][0] + J2[0][2][0];

    const dvec3 a1 = {A1[0][0], A1[1][0], A1[2][0]};
    const dvec3 a2 = {A1[0][1], A1[1][1], A1[2][1]};
    const dvec3 a3 = {A1[0][2], A1[1][2], A1[2][2]};

    const dvec3 b1 = {A2[0][0], A2[1][0], A2[2][0]};
    const dvec3 b2 = {A2[0][1], A2[1][1], A2[2][1]};
    const dvec3 b3 = {A2[0][2], A2[1][2], A2[2][2]};

    const dmat3 C = dot(transpose(A1), A2);
    const double c11 = C[0][0], c12 = C[0][1], c13 = C[0][2];
    const double c21 = C[1][0], c22 = C[1][1], c23 = C[1][2];
    const double c31 = C[2][0], c32 = C[2][1], c33 = C[2][2];

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l1 = dot(ru,a1);
    const double m1 = dot(ru,a2);
    const double n1 = dot(ru,a3);

    const double l2 = dot(ru,b1);
    const double m2 = dot(ru,b2);
    const double n2 = dot(ru,b3);

    //Order 0 (Keplerian).
    const double V0 = -G*M1*M2/d;

    //Order 2.
    const double V2 = -(G*M2/(2*d*d*d))*( (1 - 3*l1*l1)*I1x + (1 - 3*m1*m1)*I1y + (1 - 3*n1*n1)*I1z ) +
                      -(G*M1/(2*d*d*d))*( (1 - 3*l2*l2)*I2x + (1 - 3*m2*m2)*I2y + (1 - 3*n2*n2)*I2z );

    //Order 3.
    const double V3 = -(G*M2/(2*d*d*d*d))*(   l1*(5*l1*l1 - 3)*J1xxx +   m1*(5*m1*m1 - 3)*J1yyy +   n1*(5*n1*n1 - 3)*J1zzz +
                                            3*m1*(5*l1*l1 - 1)*J1xxy + 3*l1*(5*m1*m1 - 1)*J1xyy + 3*n1*(5*l1*l1 - 1)*J1xxz +
                                            3*l1*(5*n1*n1 - 1)*J1xzz + 3*n1*(5*m1*m1 - 1)*J1yyz + 3*m1*(5*n1*n1 - 1)*J1yzz +
                                           30*l1*m1*n1*J1xyz ) +
                       (G*M1/(2*d*d*d*d))*(   l2*(5*l2*l2 - 3)*J2xxx +   m2*(5*m2*m2 - 3)*J2yyy +   n2*(5*n2*n2 - 3)*J2zzz +
                                            3*m2*(5*l2*l2 - 1)*J2xxy + 3*l2*(5*m2*m2 - 1)*J2xyy + 3*n2*(5*l2*l2 - 1)*J2xxz +
                                            3*l2*(5*n2*n2 - 1)*J2xzz + 3*n2*(5*m2*m2 - 1)*J2yyz + 3*m2*(5*n2*n2 - 1)*J2yzz +
                                           30*l2*m2*n2*J2xyz );
    
    //Order 4.
    const double V4 = -(G*M2/(8*d*d*d*d*d))*( (35*pow(l1,4) - 30*l1*l1 + 3)*J1xxxx + (35*pow(m1,4) - 30*m1*m1 + 3)*J1yyyy + (35*pow(n1,4) - 30*n1*n1 + 3)*J1zzzz +
                                               20*l1*m1*(7*l1*l1 - 3)*J1xxxy + 20*l1*m1*(7*m1*m1 - 3)*J1xyyy +
                                               20*l1*n1*(7*l1*l1 - 3)*J1xxxz + 20*l1*n1*(7*n1*n1 - 3)*J1xzzz +
                                               20*m1*n1*(7*m1*m1 - 3)*J1yyyz + 20*m1*n1*(7*n1*n1 - 3)*J1yzzz +
                                               6*(35*l1*l1*m1*m1 - 5*(l1*l1 + m1*m1) + 1)*J1xxyy +
                                               6*(35*l1*l1*n1*n1 - 5*(l1*l1 + n1*n1) + 1)*J1xxzz +
                                               6*(35*m1*m1*n1*n1 - 5*(m1*m1 + n1*n1) + 1)*J1yyzz +
                                               60*m1*n1*(7*l1*l1 - 1)*J1xxyz + 60*l1*n1*(7*m1*m1 - 1)*J1xyyz + 60*l1*m1*(7*n1*n1 - 1)*J1xyzz ) +
                      -(G*M1/(8*d*d*d*d*d))*( (35*pow(l2,4) - 30*l2*l2 + 3)*J2xxxx + (35*pow(m2,4) - 30*m2*m2 + 3)*J2yyyy + (35*pow(n2,4) - 30*n2*n2 + 3)*J2zzzz +
                                               20*l2*m2*(7*l2*l2 - 3)*J2xxxy + 20*l2*m2*(7*m2*m2 - 3)*J2xyyy +
                                               20*l2*n2*(7*l2*l2 - 3)*J2xxxz + 20*l2*n2*(7*n2*n2 - 3)*J2xzzz +
                                               20*m2*n2*(7*m2*m2 - 3)*J2yyyz + 20*m2*n2*(7*n2*n2 - 3)*J2yzzz +
                                               6*(35*l2*l2*m2*m2 - 5*(l2*l2 + m2*m2) + 1)*J2xxyy +
                                               6*(35*l2*l2*n2*n2 - 5*(l2*l2 + n2*n2) + 1)*J2xxzz +
                                               6*(35*m2*m2*n2*n2 - 5*(m2*m2 + n2*n2) + 1)*J2yyzz +
                                               60*m2*n2*(7*l2*l2 - 1)*J2xxyz + 60*l2*n2*(7*m2*m2 - 1)*J2xyyz + 60*l2*m2*(7*n2*n2 - 1)*J2xyzz ) +
                      -(G/(8*d*d*d*d*d))*( (6 + 12*c11*c11 - 30*(l1*l1 + l2*l2) - 120*c11*l1*l2 + 210*l1*l1*l2*l2)*J1xx*J2xx +
                                           (6 + 12*c22*c22 - 30*(m1*m1 + m2*m2) - 120*c22*m1*m2 + 210*m1*m1*m2*m2)*J1yy*J2yy +
                                           (6 + 12*c33*c33 - 30*(n1*n1 + n2*n2) - 120*c33*n1*n2 + 210*n1*n1*n2*n2)*J1zz*J2zz +
                                           (6 + 12*c12*c12 - 30*(l1*l1 + m2*m2) - 120*c12*l1*m2 + 210*l1*l1*m2*m2)*J1xx*J2yy +
                                           (6 + 12*c21*c21 - 30*(l2*l2 + m1*m1) - 120*c21*l2*m1 + 210*l2*l2*m1*m1)*J1yy*J2xx +
                                           (6 + 12*c13*c13 - 30*(l1*l1 + n2*n2) - 120*c13*l1*n2 + 210*l1*l1*n2*n2)*J1xx*J2zz +
                                           (6 + 12*c31*c31 - 30*(l2*l2 + n1*n1) - 120*c31*l2*n1 + 210*l2*l2*n1*n1)*J1zz*J2xx +
                                           (6 + 12*c23*c23 - 30*(m1*m1 + n2*n2) - 120*c23*m1*n2 + 210*m1*m1*n2*n2)*J1yy*J2zz +
                                           (6 + 12*c32*c32 - 30*(m2*m2 + n1*n1) - 120*c32*m2*n1 + 210*m2*m2*n1*n1)*J1zz*J2yy );

    const double dV_dd = -V0/d - 3*V2/d - 4*V3/d - 5*V4/d;
    const dvec3 dd_dr = r/d;

    const double dV_dl1 = 3*G*I1x*M2*l1/(d*d*d) -
             G*M2*(10*J1xxx*l1*l1 + J1xxx*(5*l1*l1 - 3) + 30*J1xxy*l1*m1 + 30*J1xxz*l1*n1 + 3*J1xyy*(5*m1*m1 - 1) + 30*J1xyz*m1*n1 + 3*J1xzz*(5*n1*n1 - 1))/(2*d*d*d*d) +
             5*G*(3*J1xx*(J2xx*(2*c11*l2 - 7*l1*l2*l2 + l1) + J2yy*(2*c12*m2 - 7*l1*m2*m2 + l1) + J2zz*(2*c13*n2 - 7*l1*n2*n2 + l1)) - M2*(J1xxxx*l1*(7*l1*l1 - 3) + 14*J1xxxy*l1*l1*m1 + J1xxxy*m1*(7*l1*l1 - 3) + 14*J1xxxz*l1*l1*n1 + J1xxxz*n1*(7*l1*l1 - 3) + 3*J1xxyy*l1*(7*m1*m1 - 1) + 42*J1xxyz*l1*m1*n1 + 3*J1xxzz*l1*(7*n1*n1 - 1) + J1xyyy*m1*(7*m1*m1 - 3) + 3*J1xyyz*n1*(7*m1*m1 - 1) + 3*J1xyzz*m1*(7*n1*n1 - 1) + J1xzzz*n1*(7*n1*n1 - 3)))/(2*d*d*d*d*d);
    
    
    const double dV_dm1 = 3*G*I1y*M2*m1/(d*d*d) -
             G*M2*(3*J1xxy*(5*l1*l1 - 1) + 30*J1xyy*l1*m1 + 30*J1xyz*l1*n1 + 10*J1yyy*m1*m1 + J1yyy*(5*m1*m1 - 3) + 30*J1yyz*m1*n1 + 3*J1yzz*(5*n1*n1 - 1))/(2*d*d*d*d) +
             5*G*(3*J1yy*(J2xx*(2*c21*l2 - 7*l2*l2*m1 + m1) + J2yy*(2*c22*m2 - 7*m1*m2*m2 + m1) + J2zz*(2*c23*n2 - 7*m1*n2*n2 + m1)) - M2*(J1xxxy*l1*(7*l1*l1 - 3) + 3*J1xxyy*m1*(7*l1*l1 - 1) + 3*J1xxyz*n1*(7*l1*l1 - 1) + 14*J1xyyy*l1*m1*m1 + J1xyyy*l1*(7*m1*m1 - 3) + 42*J1xyyz*l1*m1*n1 + 3*J1xyzz*l1*(7*n1*n1 - 1) + J1yyyy*m1*(7*m1*m1 - 3) + 14*J1yyyz*m1*m1*n1 + J1yyyz*n1*(7*m1*m1 - 3) + 3*J1yyzz*m1*(7*n1*n1 - 1) + J1yzzz*n1*(7*n1*n1 - 3)))/(2*d*d*d*d*d);
    
    
    const double dV_dn1 = 3*G*I1z*M2*n1/(d*d*d) -
             G*M2*(3*J1xxz*(5*l1*l1 - 1) + 30*J1xyz*l1*m1 + 30*J1xzz*l1*n1 + 3*J1yyz*(5*m1*m1 - 1) + 30*J1yzz*m1*n1 + 10*J1zzz*n1*n1 + J1zzz*(5*n1*n1 - 3))/(2*d*d*d*d) +
             5*G*(3*J1zz*(J2xx*(2*c31*l2 - 7*l2*l2*n1 + n1) + J2yy*(2*c32*m2 - 7*m2*m2*n1 + n1) + J2zz*(2*c33*n2 - 7*n1*n2*n2 + n1)) - M2*(J1xxxz*l1*(7*l1*l1 - 3) + 3*J1xxyz*m1*(7*l1*l1 - 1) + 3*J1xxzz*n1*(7*l1*l1 - 1) + 3*J1xyyz*l1*(7*m1*m1 - 1) + 42*J1xyzz*l1*m1*n1 + 14*J1xzzz*l1*n1*n1 + J1xzzz*l1*(7*n1*n1 - 3) + J1yyyz*m1*(7*m1*m1 - 3) + 3*J1yyzz*n1*(7*m1*m1 - 1) + 14*J1yzzz*m1*n1*n1 + J1yzzz*m1*(7*n1*n1 - 3) + J1zzzz*n1*(7*n1*n1 - 3)))/(2*d*d*d*d*d);
    
    
    const dvec3 dl1_dr = (a1*d - r*l1)/(d*d);
    const dvec3 dm1_dr = (a2*d - r*m1)/(d*d);
    const dvec3 dn1_dr = (a3*d - r*n1)/(d*d);

    const double dV_dl2 = 3*G*I2x*M1*l2/(d*d*d) +
                    G*M1*(10*J2xxx*l2*l2 + J2xxx*(5*l2*l2 - 3) + 30*J2xxy*l2*m2 + 30*J2xxz*l2*n2 + 3*J2xyy*(5*m2*m2 - 1) + 30*J2xyz*m2*n2 + 3*J2xzz*(5*n2*n2 - 1))/(2*d*d*d*d) +
                    5*G*(3*J2xx*(J1xx*(2*c11*l1 - 7*l1*l1*l2 + l2) + J1yy*(2*c21*m1 - 7*l2*m1*m1 + l2) + J1zz*(2*c31*n1 - 7*l2*n1*n1 + l2)) - M1*(J2xxxx*l2*(7*l2*l2 - 3) + 14*J2xxxy*l2*l2*m2 + J2xxxy*m2*(7*l2*l2 - 3) + 14*J2xxxz*l2*l2*n2 + J2xxxz*n2*(7*l2*l2 - 3) + 3*J2xxyy*l2*(7*m2*m2 - 1) + 42*J2xxyz*l2*m2*n2 + 3*J2xxzz*l2*(7*n2*n2 - 1) + J2xyyy*m2*(7*m2*m2 - 3) + 3*J2xyyz*n2*(7*m2*m2 - 1) + 3*J2xyzz*m2*(7*n2*n2 - 1) + J2xzzz*n2*(7*n2*n2 - 3)))/(2*d*d*d*d*d);
    
    
    const double dV_dm2 = 3*G*I2y*M1*m2/(d*d*d) +
                    G*M1*(3*J2xxy*(5*l2*l2 - 1) + 30*J2xyy*l2*m2 + 30*J2xyz*l2*n2 + 10*J2yyy*m2*m2 + J2yyy*(5*m2*m2 - 3) + 30*J2yyz*m2*n2 + 3*J2yzz*(5*n2*n2 - 1))/(2*d*d*d*d) +
                    5*G*(3*J2yy*(J1xx*(2*c12*l1 - 7*l1*l1*m2 + m2) + J1yy*(2*c22*m1 - 7*m1*m1*m2 + m2) + J1zz*(2*c32*n1 - 7*m2*n1*n1 + m2)) - M1*(J2xxxy*l2*(7*l2*l2 - 3) + 3*J2xxyy*m2*(7*l2*l2 - 1) + 3*J2xxyz*n2*(7*l2*l2 - 1) + 14*J2xyyy*l2*m2*m2 + J2xyyy*l2*(7*m2*m2 - 3) + 42*J2xyyz*l2*m2*n2 + 3*J2xyzz*l2*(7*n2*n2 - 1) + J2yyyy*m2*(7*m2*m2 - 3) + 14*J2yyyz*m2*m2*n2 + J2yyyz*n2*(7*m2*m2 - 3) + 3*J2yyzz*m2*(7*n2*n2 - 1) + J2yzzz*n2*(7*n2*n2 - 3)))/(2*d*d*d*d*d);
    
    
    const double dV_dn2 = 3*G*I2z*M1*n2/(d*d*d) +
                    G*M1*(3*J2xxz*(5*l2*l2 - 1) + 30*J2xyz*l2*m2 + 30*J2xzz*l2*n2 + 3*J2yyz*(5*m2*m2 - 1) + 30*J2yzz*m2*n2 + 10*J2zzz*n2*n2 + J2zzz*(5*n2*n2 - 3))/(2*d*d*d*d) +
                    5*G*(3*J2zz*(J1xx*(2*c13*l1 - 7*l1*l1*n2 + n2) + J1yy*(2*c23*m1 - 7*m1*m1*n2 + n2) + J1zz*(2*c33*n1 - 7*n1*n1*n2 + n2)) - M1*(J2xxxz*l2*(7*l2*l2 - 3) + 3*J2xxyz*m2*(7*l2*l2 - 1) + 3*J2xxzz*n2*(7*l2*l2 - 1) + 3*J2xyyz*l2*(7*m2*m2 - 1) + 42*J2xyzz*l2*m2*n2 + 14*J2xzzz*l2*n2*n2 + J2xzzz*l2*(7*n2*n2 - 3) + J2yyyz*m2*(7*m2*m2 - 3) + 3*J2yyzz*n2*(7*m2*m2 - 1) + 14*J2yzzz*m2*n2*n2 + J2yzzz*m2*(7*n2*n2 - 3) + J2zzzz*n2*(7*n2*n2 - 3)))/(2*d*d*d*d*d);
    
    const dvec3 dl2_dr = (b1*d - r*l2)/(d*d);
    const dvec3 dm2_dr = (b2*d - r*m2)/(d*d);
    const dvec3 dn2_dr = (b3*d - r*n2)/(d*d);

    //F = -grad(V)
    const dvec3 force = -(dV_dd*dd_dr + dV_dl1*dl1_dr + dV_dm1*dm1_dr + dV_dn1*dn1_dr +
                                        dV_dl2*dl2_dr + dV_dm2*dm2_dr + dV_dn2*dn2_dr);

    const dvec3 dl1_da1 = ru, dm1_da2 = ru, dn1_da3 = ru;

    const double dV_dc11 = -G*J1xx*J2xx*(24*c11 - 120*l1*l2)/(8*d*d*d*d*d);
    const double dV_dc12 = -G*J1xx*J2yy*(24*c12 - 120*l1*m2)/(8*d*d*d*d*d);
    const double dV_dc13 = -G*J1xx*J2zz*(24*c13 - 120*l1*n2)/(8*d*d*d*d*d);

    const double dV_dc21 = -G*J1yy*J2xx*(24*c21 - 120*l2*m1)/(8*d*d*d*d*d);
    const double dV_dc22 = -G*J1yy*J2yy*(24*c22 - 120*m1*m2)/(8*d*d*d*d*d);
    const double dV_dc23 = -G*J1yy*J2zz*(24*c23 - 120*m1*n2)/(8*d*d*d*d*d);

    const double dV_dc31 = -G*J1zz*J2xx*(24*c31 - 120*l2*n1)/(8*d*d*d*d*d);
    const double dV_dc32 = -G*J1zz*J2yy*(24*c32 - 120*m2*n1)/(8*d*d*d*d*d);
    const double dV_dc33 = -G*J1zz*J2zz*(24*c33 - 120*n1*n2)/(8*d*d*d*d*d);

    const dvec3 dc11_da1 = b1, dc21_da2 = b1, dc31_da3 = b1;
    const dvec3 dc12_da1 = b2, dc22_da2 = b2, dc32_da3 = b2;
    const dvec3 dc13_da1 = b3, dc23_da2 = b3, dc33_da3 = b3;

    const dvec3 dV_da1 = dV_dl1*dl1_da1 + dV_dc11*dc11_da1 + dV_dc12*dc12_da1 + dV_dc13*dc13_da1;
    const dvec3 dV_da2 = dV_dm1*dm1_da2 + dV_dc21*dc21_da2 + dV_dc22*dc22_da2 + dV_dc23*dc23_da2;
    const dvec3 dV_da3 = dV_dn1*dn1_da3 + dV_dc31*dc31_da3 + dV_dc32*dc32_da3 + dV_dc33*dc33_da3;

    const dvec3 tau1 = -cross(a1, dV_da1) - cross(a2, dV_da2) - cross(a3, dV_da3);

    return {force[0],force[1],force[2], tau1[0],tau1[1],tau1[2]};
}

//Mutual force of 2 rigid bodies AND the torque felt by the primary, assuming mascon distributions with constant densities.
dvec6 mut_force_torque1_masc(const dvec3 &r, const double M1, const dmatnx3 &masc1, const dmat3 &A1,
                                             const double M2, const dmatnx3 &masc2, const dmat3 &A2)
{
    #ifdef _OPENMP
        const int total_threads = omp_get_max_threads();
        const int used_threads  = (total_threads > 1 ? total_threads/2 : 1);
    #else
        constexpr int used_threads = 1;
    #endif
    (void)used_threads;

    size_t i,j, N1 = masc1.size(), N2 = masc2.size();
    double sumfx = 0.0, sumfy = 0.0, sumfz = 0.0, sumtx = 0.0, sumty = 0.0, sumtz = 0.0;
    #ifdef _OPENMP
        #pragma omp parallel for reduction(+:sumfx,sumfy,sumfz,sumtx,sumty,sumtz)\
                                 schedule(dynamic)\
                                 num_threads(used_threads)
    #endif
    for (i = 0; i < N1; ++i)
    {
        const dvec3 a1i = dot(A1, masc1[i]);
        for (j = 0; j < N2; ++j)
        {
            const dvec3 dij = r + dot(A2, masc2[j]) - a1i;
            const dvec3 cp = cross(a1i,dij);
            const double len = length(dij);
            const double invlen3 = 1.0/(len*len*len);
            sumfx += dij[0]*invlen3;
            sumfy += dij[1]*invlen3;
            sumfz += dij[2]*invlen3;
            sumtx += cp[0]*invlen3;
            sumty += cp[1]*invlen3;
            sumtz += cp[2]*invlen3;
        }
    }
    const double coeff = G*M1*M2/double(N1*N2);
    return {-coeff*sumfx,-coeff*sumfy,-coeff*sumfz, coeff*sumtx,coeff*sumty,coeff*sumtz};
}

/* End of gravity force and torque expressions. */

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/* Gravity acceleration expressions. */

//Acceleration of a test particle at position r, due to a rigid body, assuming inertial integral expansion of order 2 approximation.
dvec3 accel_integrals_ord2(const dvec3 &r, const double M, const dtens &J, const dmat3 &A)
{
    const double Ix = J[0][2][0] + J[0][0][2];
    const double Iy = J[2][0][0] + J[0][0][2];
    const double Iz = J[2][0][0] + J[0][2][0];

    const dvec3 a1 = {A[0][0], A[1][0], A[2][0]};
    const dvec3 a2 = {A[0][1], A[1][1], A[2][1]};
    const dvec3 a3 = {A[0][2], A[1][2], A[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l = dot(ru,a1);
    const double m = dot(ru,a2);
    const double n = dot(ru,a3);

    //Order 0 (Keplerian).
    const double V0 = -G*M/d;

    //Order 2.
    const double V2 = -(G/(2*d*d*d))*( (1 - 3*l*l)*Ix + (1 - 3*m*m)*Iy + (1 - 3*n*n)*Iz );

    const double dV_dd = -V0/d - 3*V2/d;
    const dvec3 dd_dr = ru;

    const double dV_dl = 3*G*Ix*l/(d*d*d);
    const double dV_dm = 3*G*Iy*m/(d*d*d);
    const double dV_dn = 3*G*Iz*n/(d*d*d);
    const dvec3 dl_dr  = (a1*d - r*l)/(d*d);
    const dvec3 dm_dr  = (a2*d - r*m)/(d*d);
    const dvec3 dn_dr  = (a3*d - r*n)/(d*d);

    return -(dV_dd*dd_dr + dV_dl*dl_dr + dV_dm*dm_dr + dV_dn*dn_dr);
}

//Acceleration of a test particle at position r, due to a rigid body, assuming inertial integral expansion of order 3 approximation.
dvec3 accel_integrals_ord3(const dvec3 &r, const double M, const dtens &J, const dmat3 &A)
{
    const double Jxxx = J[3][0][0];
    const double Jyyy = J[0][3][0];
    const double Jzzz = J[0][0][3];
    const double Jxxy = J[2][1][0];
    const double Jxyy = J[1][2][0];
    const double Jxxz = J[2][0][1];
    const double Jxzz = J[1][0][2];
    const double Jyyz = J[0][2][1];
    const double Jyzz = J[0][1][2];
    const double Jxyz = J[1][1][1];
    
    const double Ix = J[0][2][0] + J[0][0][2];
    const double Iy = J[2][0][0] + J[0][0][2];
    const double Iz = J[2][0][0] + J[0][2][0];

    const dvec3 a1 = {A[0][0], A[1][0], A[2][0]};
    const dvec3 a2 = {A[0][1], A[1][1], A[2][1]};
    const dvec3 a3 = {A[0][2], A[1][2], A[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l = dot(ru,a1);
    const double m = dot(ru,a2);
    const double n = dot(ru,a3);

    //Order 0 (Keplerian).
    const double V0 = -G*M/d;

    //Order 2.
    const double V2 = -(G/(2*d*d*d))*( (1 - 3*l*l)*Ix + (1 - 3*m*m)*Iy + (1 - 3*n*n)*Iz );

    //Order 3.
    const double V3 = -(G/(2*d*d*d*d))*(    l*(5*l*l - 3)*Jxxx +   m*(5*m*m - 3)*Jyyy +   n*(5*n*n - 3)*Jzzz +
                                          3*m*(5*l*l - 1)*Jxxy + 3*l*(5*m*m - 1)*Jxyy + 3*n*(5*l*l - 1)*Jxxz +
                                          3*l*(5*n*n - 1)*Jxzz + 3*n*(5*m*m - 1)*Jyyz + 3*m*(5*n*n - 1)*Jyzz +
                                         30*l*m*n*Jxyz );

    const double dV_dd = -V0/d - 3*V2/d - 4*V3/d;
    const dvec3 dd_dr = ru;

    const double dV_dl = 3*G*Ix*l/(d*d*d) - G*(10*Jxxx*l*l + Jxxx*(5*l*l - 3) + 30*Jxxy*l*m + 30*Jxxz*l*n + 3*Jxyy*(5*m*m - 1) + 30*Jxyz*m*n + 3*Jxzz*(5*n*n - 1))/(2*d*d*d*d);
    const double dV_dm = 3*G*Iy*m/(d*d*d) - G*(3*Jxxy*(5*l*l - 1) + 30*Jxyy*l*m + 30*Jxyz*l*n + 10*Jyyy*m*m + Jyyy*(5*m*m - 3) + 30*Jyyz*m*n + 3*Jyzz*(5*n*n - 1))/(2*d*d*d*d);
    const double dV_dn = 3*G*Iz*n/(d*d*d) - G*(3*Jxxz*(5*l*l - 1) + 30*Jxyz*l*m + 30*Jxzz*l*n + 3*Jyyz*(5*m*m - 1) + 30*Jyzz*m*n + 10*Jzzz*n*n + Jzzz*(5*n*n - 3))/(2*d*d*d*d);
    const dvec3 dl_dr = (a1*d - r*l)/(d*d);
    const dvec3 dm_dr = (a2*d - r*m)/(d*d);
    const dvec3 dn_dr = (a3*d - r*n)/(d*d);

    return -(dV_dd*dd_dr + dV_dl*dl_dr + dV_dm*dm_dr + dV_dn*dn_dr);
}

//Acceleration of a test particle at position r, due to a rigid body, assuming inertial integral expansion of order 4 approximation.
dvec3 accel_integrals_ord4(const dvec3 &r, const double M, const dtens &J, const dmat3 &A)
{
    const double Jxxx = J[3][0][0];
    const double Jyyy = J[0][3][0];
    const double Jzzz = J[0][0][3];
    const double Jxxy = J[2][1][0];
    const double Jxyy = J[1][2][0];
    const double Jxxz = J[2][0][1];
    const double Jxzz = J[1][0][2];
    const double Jyyz = J[0][2][1];
    const double Jyzz = J[0][1][2];
    const double Jxyz = J[1][1][1];

    const double Jxxxx = J[4][0][0];
    const double Jyyyy = J[0][4][0];
    const double Jzzzz = J[0][0][4];
    const double Jxxxy = J[3][1][0];
    const double Jxyyy = J[1][3][0];
    const double Jxxxz = J[3][0][1];
    const double Jxzzz = J[1][0][3];
    const double Jyyyz = J[0][3][1];
    const double Jyzzz = J[0][1][3];
    const double Jxxyy = J[2][2][0];
    const double Jxxzz = J[2][0][2];
    const double Jyyzz = J[0][2][2];
    const double Jxxyz = J[2][1][1];
    const double Jxyyz = J[1][2][1];
    const double Jxyzz = J[1][1][2];
    
    const double Ix = J[0][2][0] + J[0][0][2];
    const double Iy = J[2][0][0] + J[0][0][2];
    const double Iz = J[2][0][0] + J[0][2][0];

    const dvec3 a1 = {A[0][0], A[1][0], A[2][0]};
    const dvec3 a2 = {A[0][1], A[1][1], A[2][1]};
    const dvec3 a3 = {A[0][2], A[1][2], A[2][2]};

    const double d = sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    const dvec3 ru = r/d;

    const double l = dot(ru,a1);
    const double m = dot(ru,a2);
    const double n = dot(ru,a3);

    //Order 0 (Keplerian).
    const double V0 = -G*M/d;

    //Order 2.
    const double V2 = -(G/(2*d*d*d))*( (1 - 3*l*l)*Ix + (1 - 3*m*m)*Iy + (1 - 3*n*n)*Iz );

    //Order 3.
    const double V3 = -(G/(2*d*d*d*d))*(    l*(5*l*l - 3)*Jxxx +   m*(5*m*m - 3)*Jyyy +   n*(5*n*n - 3)*Jzzz +
                                          3*m*(5*l*l - 1)*Jxxy + 3*l*(5*m*m - 1)*Jxyy + 3*n*(5*l*l - 1)*Jxxz +
                                          3*l*(5*n*n - 1)*Jxzz + 3*n*(5*m*m - 1)*Jyyz + 3*m*(5*n*n - 1)*Jyzz +
                                         30*l*m*n*Jxyz );

    //Order 4.
    const double V4 = -(G/(8*d*d*d*d*d))*( (35*pow(l,4) - 30*l*l + 3)*Jxxxx + (35*pow(m,4) - 30*m*m + 3)*Jyyyy + (35*pow(n,4) - 30*n*n + 3)*Jzzzz +
                                            20*l*m*(7*l*l - 3)*Jxxxy + 20*l*m*(7*m*m - 3)*Jxyyy +
                                            20*l*n*(7*l*l - 3)*Jxxxz + 20*l*n*(7*n*n - 3)*Jxzzz +
                                            20*m*n*(7*m*m - 3)*Jyyyz + 20*m*n*(7*n*n - 3)*Jyzzz +
                                            6*(35*l*l*m*m - 5*(l*l + m*m) + 1)*Jxxyy +
                                            6*(35*l*l*n*n - 5*(l*l + n*n) + 1)*Jxxzz +
                                            6*(35*m*m*n*n - 5*(m*m + n*n) + 1)*Jyyzz +
                                            60*m*n*(7*l*l - 1)*Jxxyz + 60*l*n*(7*m*m - 1)*Jxyyz + 60*l*m*(7*n*n - 1)*Jxyzz );

    const double dV_dd = -V0/d - 3*V2/d - 4*V3/d - 5*V4/d;
    const dvec3 dd_dr = ru;

    const double dV_dl = 3*G*Ix*l/(d*d*d) - G*(10*Jxxx*l*l + Jxxx*(5*l*l - 3) + 30*Jxxy*l*m + 30*Jxxz*l*n + 3*Jxyy*(5*m*m - 1) + 30*Jxyz*m*n + 3*Jxzz*(5*n*n - 1))/(2*d*d*d*d) - G*(Jxxxx*(140*l*l*l - 60*l) + 280*Jxxxy*l*l*m + 20*Jxxxy*m*(7*l*l - 3) + 280*Jxxxz*l*l*n + 20*Jxxxz*n*(7*l*l - 3) + Jxxyy*(420*l*m*m - 60*l) + 840*Jxxyz*l*m*n + Jxxzz*(420*l*n*n - 60*l) + 20*Jxyyy*m*(7*m*m - 3) + 60*Jxyyz*n*(7*m*m - 1) + 60*Jxyzz*m*(7*n*n - 1) + 20*Jxzzz*n*(7*n*n - 3))/(8*d*d*d*d*d);
    const double dV_dm = 3*G*Iy*m/(d*d*d) - G*(3*Jxxy*(5*l*l - 1) + 30*Jxyy*l*m + 30*Jxyz*l*n + 10*Jyyy*m*m + Jyyy*(5*m*m - 3) + 30*Jyyz*m*n + 3*Jyzz*(5*n*n - 1))/(2*d*d*d*d) - G*(20*Jxxxy*l*(7*l*l - 3) + Jxxyy*(420*l*l*m - 60*m) + 60*Jxxyz*n*(7*l*l - 1) + 280*Jxyyy*l*m*m + 20*Jxyyy*l*(7*m*m - 3) + 840*Jxyyz*l*m*n + 60*Jxyzz*l*(7*n*n - 1) + Jyyyy*(140*m*m*m - 60*m) + 280*Jyyyz*m*m*n + 20*Jyyyz*n*(7*m*m - 3) + Jyyzz*(420*m*n*n - 60*m) + 20*Jyzzz*n*(7*n*n - 3))/(8*d*d*d*d*d);
    const double dV_dn = 3*G*Iz*n/(d*d*d) - G*(3*Jxxz*(5*l*l - 1) + 30*Jxyz*l*m + 30*Jxzz*l*n + 3*Jyyz*(5*m*m - 1) + 30*Jyzz*m*n + 10*Jzzz*n*n + Jzzz*(5*n*n - 3))/(2*d*d*d*d) - G*(20*Jxxxz*l*(7*l*l - 3) + 60*Jxxyz*m*(7*l*l - 1) + Jxxzz*(420*l*l*n - 60*n) + 60*Jxyyz*l*(7*m*m - 1) + 840*Jxyzz*l*m*n + 280*Jxzzz*l*n*n + 20*Jxzzz*l*(7*n*n - 3) + 20*Jyyyz*m*(7*m*m - 3) + Jyyzz*(420*m*m*n - 60*n) + 280*Jyzzz*m*n*n + 20*Jyzzz*m*(7*n*n - 3) + Jzzzz*(140*n*n*n - 60*n))/(8*d*d*d*d*d);
    const dvec3 dl_dr = (a1*d - r*l)/(d*d);
    const dvec3 dm_dr = (a2*d - r*m)/(d*d);
    const dvec3 dn_dr = (a3*d - r*n)/(d*d);

    return -(dV_dd*dd_dr + dV_dl*dl_dr + dV_dm*dm_dr + dV_dn*dn_dr);
}

//Acceleration of a test particle at position r, due to a rigid body, assuming mascons distribution with constant density.
dvec3 accel_masc(const dvec3 &r, const double M, const dmatnx3 &masc, const dmat3 &A)
{
    #ifdef _OPENMP
        const int total_threads = omp_get_max_threads();
        const int used_threads  = (total_threads > 1 ? total_threads/2 : 1);
    #else
        constexpr int used_threads = 1;
    #endif
    (void)used_threads;


    size_t i, N = masc.size();
    double sumfx = 0.0, sumfy = 0.0, sumfz = 0.0;
    #ifdef _OPENMP
        #pragma omp parallel for reduction(+:sumfx,sumfy,sumfz)\
                                 schedule(dynamic)\
                                 num_threads(used_threads)
    #endif
    for (i = 0; i < N; ++i)
    {
        const dvec3 ri = dot(A, masc[i]);
        const dvec3 fi = (r - ri)/pow(length(r - ri), 3.0);
        sumfx += fi[0];
        sumfy += fi[1];
        sumfz += fi[2];

    }
    return -G*M*dvec3{sumfx,sumfy,sumfz}/double(N);
}

/* End of gravity acceleration expressions. */

//Acceleration of a body (e.g. spacecraft) due to SRP - cannonball model.
dvec3 accel_srp(const double rho, const double A, const double m, const dvec3 &rbody, const dvec3 &rsun)
{
    const dvec3 dr = rbody - rsun; //Sun to spacecraft vector.
    const double drlen = length(dr);
    return (1.0 + rho)*(A/m)*4.56e-9*AU2KM*AU2KM*dr/(drlen*drlen*drlen); //[km/sec^2]
}

#endif
