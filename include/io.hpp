...
public:
    ...
    void export_json()
    {
        ...
        if (ell_checkbox)
        {
            jsonparams["Shape models"] = "Ellipsoids";
            jsonparams["Semiaxes"]["a1"] = semiaxes1[0];
            jsonparams["Semiaxes"]["b1"] = semiaxes1[1];
            jsonparams["Semiaxes"]["c1"] = semiaxes1[2];
            jsonparams["Semiaxes"]["a2"] = semiaxes2[0];
            jsonparams["Semiaxes"]["b2"] = semiaxes2[1];
            jsonparams["Semiaxes"]["c2"] = semiaxes2[2];
        }
        else
        {
            jsonparams["Shape models"] = ".obj files";
            if (clicked_masc1_index != -1)
            {
                jsonparams["Body 1 shape nature"] = "Mascon model";
                jsonparams["Path"] = obj_path1;
            }
            else
            {
                jsonparams["Body 1 shape nature"] = "Polyhedron model";
                jsonparams["Path"] = obj_path1;
            }
