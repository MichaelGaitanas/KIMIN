...
private:
    double M1,M2; //Masses.
    dvec6 cart; //Cartesian state (mutual position/velocity).
    dvec4 q1, q2; //Quaternions.
    dvec3 w1i, w2i, w1b, w2b; //Angular velocities in the inertial and body frames.
    bool ord2_checkbox, ord3_checkbox; //ord4_checkbox is not needed, coz there's no other option available if the other 2 are false (none of them means that the integrator won't run).
    ...
    dmat orbit_sol; //Final matrix that contains all the orbital data.
...
public:
    ...
    void prepare(const properties_panel &properties, std::atomic<bool> &abort_flag, std::atomic<float> &progress)
    {
        ...
        if (properties.impactor_checkbox)
        {
            ...
            cart[3] += properties.beta*properties.M_impact*properties.v_impact[0]/properties.M2;
            cart[4] += properties.beta*properties.M_impact*properties.v_impact[1]/properties.M2;
            cart[5] += properties.beta*properties.M_impact*properties.v_impact[2]/properties.M2;
        }
