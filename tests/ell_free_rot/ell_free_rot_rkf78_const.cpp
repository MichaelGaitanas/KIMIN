...
int main()
{
    ...
    //Time parameters.
    double t, t0 = 0.0; //[sec]
    double tmax = 20*3600.0; //[sec]
    double dt = 5.0; //[sec]

    //Initial conditions (orientation and angular velocity).
    dvec4 q  = ang2quat(dvec3{0.03*pi/8,pi/2.0,0.1*pi}); //[ ]
    dvec3 wi = dvec3{0.0, -0.0001, 0.002}; //[rad/sec]

    q = quat2unit(q);
    dvec3 wb = iner2body(wi, quat2mat(q));

    boost::array<double, 7> state = { q[0],q[1],q[2],q[3],  wb[0],wb[1],wb[2] };
    boost::numeric::odeint::runge_kutta_fehlberg78<boost::array<double, 7>> rkf78_const;
